...
\begin{quote}
	\textbf{RQ1:} \emph{What are the runtime savings of asynchronous duet method compared to the sequential method?}
\end{quote}
...
\xxx{Consider including actual example of speedup where workload A is 2x faster than B, but B runs alone at the end so B is actually only 1.5x faster.}
...
\xxx{Consider nature of long vs. short outside interference and its frequency, very short messes us up while very long is fine for duet and seqn depending on RMIT runtime. There is some sweet spot that duet will perform better with compared to seqn.}
...
\begin{description}
	\item[Accessibility]
		Asynchronous duet relaxes the synchronized iteration requirement on benchmark harnesses imposed by synchronized duet method.
	...
	\item[Accuracy]
		Synchronized duet method can reduce the variance of benchmark results measured in a variable environment, such as a public cloud~\cite{bulej2020duet}.
