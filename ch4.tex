...
Different statistical methods such as confidence intervals and hypothesis testing are used to evaluate if the performance of A/B labeled runs is the same.
In the context of duet runs A/B labeling is performed on concurrently running duet pairs.

\section{Data filtering}
\label{sec:data_filtering}

In our experiments, we removed the first half of all iterations per run of Java benchmarks to avoid warm-up, same as in~\citet{bulej2020duet}.
